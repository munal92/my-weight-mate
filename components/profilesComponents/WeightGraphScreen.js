...
const WeightGraphScreen = () => {
  ...
  const [current3Months, setCurrent3Months] = useState(moment().startOf('month').subtract(2, 'months'));
  ...
  const filterData = (period) => {
    ...
    switch (period) {
      ...
      case '3months': {
        ...
        const endOf3Months = current3Months.clone().add(2, 'months').endOf('month');
        filtered = filtered.filter((entry) => {
          const entryDate = moment(entry.date);
  ...
  const handleDataPointClick = (data) => {
    ...
    Alert.alert('Weight Value', `Date: ${data.indexLabel}\nWeight: ${data.value}`);
  };

  // Render period label
  const renderPeriodLabel = () => {
    if (selectedPeriod === 'weekly') {
      const startOfWeek = currentWeek.clone().startOf('isoWeek').format('MMM D');
        ...
        {['weekly', 'monthly', '3months', 'yearly'].map((period) => (
          <TouchableOpacity
            ...
            style={{
              marginRight: 20,
              padding: 10,
              backgroundColor: selectedPeriod === period ? '#4CAF50' : '#ddd',
              borderRadius: 5,
            }}
            onPress={() => {
